...
创建共享的 Prisma 客户端实例 `src/lib/prisma.ts`：
...
如果遇到数据库连接错误，检查 `.env` 文件中的数据库用户名是否与 Docker 容器配置一致：

```bash
# 检查容器的实际用户名
docker inspect pg16 | grep -E "POSTGRES_(USER|PASSWORD|DB)"
```

例如，如果容器配置是 `POSTGRES_USER=myuser`，则 `.env` 中应该是：
```bash
DATABASE_URL="postgresql://myuser:mypassword@localhost:5925/whitenote?schema=public"
```
...
export default defineConfig({
  ...
  migrations: {
    ...
    seed: "tsx ./prisma/seed.ts",  // 在这里配置种子脚本
  },
  datasource: {
...
✅ Created owner user: owner@whitenote.local
✅ Created AI config for user: owner@whitenote.local
✅ Created built-in templates: 2
✅ Created sample tags: 5
...
# 4. 直接查询数据库验证数据
docker exec pg16 psql -U myuser -d whitenote -c "SELECT email, name FROM \"User\";"
docker exec pg16 psql -U myuser -d whitenote -c "SELECT id, name, \"isBuiltIn\" FROM \"Template\";"
```
